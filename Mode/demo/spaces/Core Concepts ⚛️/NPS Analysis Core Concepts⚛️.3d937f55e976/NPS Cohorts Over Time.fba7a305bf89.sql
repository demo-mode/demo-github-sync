  ...
  admin_status AS (
    SELECT 'd - Admin status' AS cohort_type,
      ...
      FROM  roberto_salcido899.solutionsdemorollupperiodsv3 rp
      JOIN  roberto_salcido899.solutionsdemonpsscoresv3 ns
        ON  ns.created_at >= rp.starts_at
       AND  ns.created_at < rp.ends_at
     WHERE rp.period_id = 1090
     GROUP BY 1,2,3,4
  ...
  SELECT time_id,
         cohort_type AS ordered_cohort_type,
    ...
    FROM metrics
    WHERE RESPONSES > 35
    AND time_id >= '2019-04-01'
      AND time_id < CURRENT_DATE - 1
