  ...
  metrics AS (
    ...
    SELECT time_id,
         MAX(CASE WHEN period_id = 1 THEN nps*100 ELSE NULL END) / 2 AS all_time_nps,
         MAX(CASE WHEN period_id = 1007 THEN nps*100 ELSE NULL END) / 2 AS d7_nps, 
         MAX(CASE WHEN period_id = 1028 THEN nps*100 ELSE NULL END) / 2 AS d28_nps, 
         MAX(CASE WHEN period_id = 1090 THEN nps*100 ELSE NULL END) / 2 AS d90_nps 
    FROM metrics
   WHERE time_id = DATE_TRUNC('day',CURRENT_TIMESTAMP)
      OR time_id = DATE_TRUNC('day',CURRENT_TIMESTAMP) - INTERVAL '28 DAY'
   GROUP BY 1
   ORDER BY 1